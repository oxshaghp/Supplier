    ...
    <div className="space-y-8">
          ...
          <div key={key} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                ...
                key === 'users' ? 'bg-blue-100' :
                key === 'businesses' ? 'bg-purple-100' : 'bg-yellow-100'
              }`}>
                <i className={`text-xl ${
                  key === 'revenue' ? 'ri-money-dollar-circle-line text-green-600' :
                  key === 'users' ? 'ri-user-line text-blue-600' :
                  key === 'businesses' ? 'ri-store-line text-purple-600' : 'ri-vip-crown-line text-yellow-600'
                }`}></i>
              </div>
              <span className={`text-sm font-medium px-2 py-1 rounded-full ${
                data.growth > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
              }`}>
            ...
            <div>
              <h3 className="text-2xl font-bold text-gray-800">
                {key === 'revenue' ? formatCurrency(data.current) : formatNumber(data.current)}
              </h3>
              <p className="text-gray-600 text-sm capitalize">
                {key === 'businesses' ? 'Total Businesses' : 
                 key === 'subscriptions' ? 'Paid Subscriptions' : 
                 `Total ${key}`}
              </p>
            </div>
              ...
              {data.data.map((value, index) => (
                <div
                  ...
                  className={`flex-1 rounded-t ${
                    key === 'revenue' ? 'bg-green-200' :
                    key === 'users' ? 'bg-blue-200' :
                    key === 'businesses' ? 'bg-purple-200' : 'bg-yellow-200'
                  }`}
                  style={{
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-6">Revenue by Plan</h3>
            ...
            {revenueByPlan.map((plan, index) => (
              <div key={index} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-800">{formatCurrency(plan.revenue)}</p>
                  <div className="w-32 bg-gray-200 rounded-full h-2 mt-1">
                    <div
                      className={`h-2 rounded-full ${plan.color}`}
                      style={{
                        width: `${(plan.revenue / Math.max(...revenueByPlan.map(p => p.revenue))) * 100}%`
                      }}
                    ></div>
                  </div>
                </div>
        ...
        {/* Top Categories */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-6">Top Business Categories</h3>
          <div className="space-y-4">
              ...
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{category.businesses} businesses</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-gray-800">{category.revenue}</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-6">Daily User Activity (Last 7 Days)</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-700">Date</th>
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-700">New Users</th>
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-700">Active Users</th>
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-700">Revenue</th>
                <th className="text-left py-3 px-6 text-sm font-medium text-gray-700">Activity</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {userActivity.map((activity, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    <span className="font-medium text-gray-800">{formatNumber(activity.activeUsers)}</span>
                  </td>
                  <td className="py-4 px-6">
                    <span className="font-medium text-gray-800">{formatCurrency(activity.revenue)}</span>
                  </td>
                  <td className="py-4 px-6">
                    <div className="w-24 bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${(activity.activeUsers / Math.max(...userActivity.map(a => a.activeUsers))) * 100}%`
                        }}
                      ></div>
                    </div>
                  </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h4 className="font-semibold text-gray-800">Server Performance</h4>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h4 className="font-semibold text-gray-800">Database Health</h4>
